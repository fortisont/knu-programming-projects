...
using namespace std;
string error = "Register index out of range";

Memory::Memory(): reg_amount(0), reg_list() {}
Memory::Memory(int reg_amt): reg_amount(reg_amt), reg_list(reg_amt) {}
...
Instruction::Instruction(): reg_num(-1), instr_num(0), instr_type(INS_END), left_arg(-1), right_arg(-1){}

Instruction::Instruction(int reg_num, int instr_num, int new_instr_type, int new_left_arg, int new_right_arg) 
	: reg_num(reg_num), instr_num(instr_num), instr_type(-1), left_arg(new_left_arg), right_arg(new_right_arg){
...
Instruction::Instruction(const Instruction& other): reg_num(other.get_reg_num()), instr_num(other.get_instr_num())
	, instr_type(other.get_instr_type()), left_arg(other.get_left_arg()), right_arg(other.get_right_arg()){}
...
int Instruction::exec(Memory* mem){
	...
	case INS_DECJNZ:{
		if (mem->is_reg_zero(reg_num)) {return get_right_arg();}
		mem->dec_reg(reg_num);
		return get_left_arg();
	}
	}
...
Program::Program(): instr_amt(0), instr_list() {}

Program::Program(size_t size): instr_amt(size), instr_list(size) {}

Program::Program(const Program& other): instr_amt(other.get_instr_amt()), instr_list(other.get_instr_lst()) {}
...
MinskyAutomata::MinskyAutomata(): program_lst(), mem(new Memory(10)), cur_program(0), cur_state(0) {}

MinskyAutomata::MinskyAutomata(int reg_amt): program_lst(), mem(new Memory(reg_amt)), cur_program(0), cur_state(0) {}
